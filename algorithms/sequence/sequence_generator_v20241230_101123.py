"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:11:23
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        for i in range(num_sections):
            ...
            section ^= (position + i) & ((1 << section_size) - 1)
            if i > 0:
                section ^= prev_section
            
            # Apply position-based rotation
            rotate = ((position * (i + 1)) % (section_size - 1)) + 1
        ...
        rotate_final = (position % (significant_bits - 1)) + 1
        ...
        transformed ^= (position * 0x1f) & mask
    ...
    def _apply_non_linear_transform(self, value: int, position: int) -> int:
        ...
        for i in range(num_chunks):
            ...
            # Apply position-based mixing
            chunk ^= (position * (i + 1)) & ((1 << chunk_size) - 1)
            if i > 0:
                chunk ^= prev_chunk
            
            # Apply position-based rotation
            rotate = ((position * (i + 1)) % (chunk_size - 1)) + 1
