"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:53:50
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if pos_mod == 0:
            ...
            x = (x << 2) | (x >> 30)
            x = x & ~0x15  # Clear bits 0,2,4
            x = x + 0x15
            
        elif pos_mod == 2:
            # Third position in block
            x = (x << 1) | (x >> 31)
            x = x & ~0x31  # Clear bits 0,4,5
            x = x + 0x31
            
        elif pos_mod == 3:
            # Fourth position in block
            x = (x << 4) | (x >> 28)
            x = x & ~0x4c  # Clear bits 2,3,6
            x = x + 0x4c
            
        else:
            # Fifth position in block
            x = (x << 3) | (x >> 29)
            x = x & ~0xe0  # Clear bits 5,6,7
            x = x + 0xe0
