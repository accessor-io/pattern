"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:24:08
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        shift_amount = position // 8  # Increase shift as position increases
        base_pattern = (value >> shift_amount) & mask  # Shift right before masking
        ...
        if block_pos == 0:
            ...
            result = (result + (block_num * 0x123)) & mask  # Reduced constant
            
            # Additional mixing for 5th positions
            if position > 20:
                mix = (result ^ (result >> 1)) & mask
                result = (mix + (block_num * 0x67)) & mask
        elif block_pos == 1:
            # First transition position
            shift = (block_num * 2 + 1) % significant_bits
            result = ((base_pattern >> shift) | (base_pattern << (significant_bits - shift))) & mask
            result = (result ^ (block_num * 0x234)) & mask
            
            # Enhanced mixing for later positions
            if position > 30:
                mix = (result + (result << 1)) & mask
                result = (mix ^ (block_num * 0x78)) & mask  # Reduced constant
        elif block_pos == 2:
            # Middle transition position
            shift = (block_num * 3 + 1) % significant_bits
            result = ((base_pattern << shift) ^ (base_pattern >> (significant_bits - shift))) & mask
            result = (result + (block_num * 0x345)) & mask
            
            # Additional pattern for later positions
            if position > 40:
                mix = (result ^ (result >> 1)) & mask
                result = (mix + (block_num * 0x89)) & mask  # Reduced constant
        elif block_pos == 3:
            # Late transition position
            shift = (block_num * 2 + 1) % significant_bits
            result = ((base_pattern >> shift) ^ (base_pattern << (significant_bits - shift))) & mask
            result = (result ^ (block_num * 0x456)) & mask
            
            # Enhanced mixing for later positions
            if position > 50:
                mix = (result + (result << 1)) & mask
                result = (mix ^ (block_num * 0x90)) & mask  # Reduced constant
        else:  # block_pos == 4
            # Pre-special position
            shift = (block_num * 3 + 1) % significant_bits
            result = ((base_pattern << shift) | (base_pattern >> (significant_bits - shift))) & mask
            result = (result + (block_num * 0x567)) & mask
            
            # Additional pattern for later positions
            if position > 60:
                mix = (result ^ (result >> 1)) & mask
                result = (mix + (block_num * 0xa1)) & mask  # Reduced constant
        
        # Apply final transformations based on position
        if position > 20:
            # Progressive mixing for later positions
            mix_shift = (position * 2) % significant_bits
            result = ((result << mix_shift) | (result >> (significant_bits - mix_shift))) & mask
            
            if position > 40:
                # Enhanced mixing for very late positions
                mix = (result ^ (result >> 1)) & mask
                result = (mix + (position * 0x12)) & mask  # Reduced constant
