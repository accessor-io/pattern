"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 03:22:17
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        if block_pos == 0:
            ...
            result ^= (block_num * 0x11)
        elif block_pos == 1:
            shift = (block_num * 2 + 2) % significant_bits
            result = ((result >> shift) | (result << (significant_bits - shift))) & mask
            result ^= (block_num * 0x22)
        elif block_pos == 2:
            result = (result + (block_num * 0x33)) & mask
            result ^= (position * 0x11)
        elif block_pos == 3:
            shift = (block_num + 3) % significant_bits
            result = ((result << shift) | (result >> (significant_bits - shift))) & mask
            result ^= (block_num * 0x44)
        else:  # block_pos == 4
            result = (result + (block_num * 0x55)) & mask
            result ^= (position * 0x22)
        
        # Apply final transformations based on position ranges
        if position > 50:
