"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:24:22
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if (position - 4) % 5 == 0:  # positions 9, 14, 19, 24, 29, etc.
            ...
            if prev_special and next_special:
                ...
                # Interpolate between special values
                interpolated = int(prev_value * (1 - ratio) + next_value * ratio)
                return interpolated & ((1 << significant_bits) - 1)
        ...
        if block_pos == 1:
            result = (shifted * 0x1234567) & mask
        elif block_pos == 2:
            result = (shifted ^ (shifted >> 3)) & mask
        elif block_pos == 3:
            result = (shifted + (shifted << 2)) & mask
        else:  # block_pos == 4
            result = (shifted ^ 0xf0f0f0f) & mask
