"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:16:19
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if block_pos == 0:
            ...
            result = (result + (block_num * 0x1234567)) & mask
        elif block_pos == 1:
            # First transition
            shift = (block_num * 2 + 3) % significant_bits
            result = ((base_pattern >> shift) | (base_pattern << (significant_bits - shift))) & mask
            result = (result ^ (block_num * 0x789abc)) & mask
        elif block_pos == 2:
            # Middle transition
            result = base_pattern ^ (base_pattern >> 3) ^ (block_num * 0xdef012)
            result = ((result << 2) | (result >> (significant_bits - 2))) & mask
        elif block_pos == 3:
            # Late transition
            shift = (block_num * 4 + 2) % significant_bits
            result = ((base_pattern << shift) ^ (base_pattern >> (significant_bits - shift))) & mask
            result = (result + (block_num * 0x345678)) & mask
        else:  # block_pos == 4
            # Pre-special position
            shift = (block_num * 5 + 1) % significant_bits
            result = ((base_pattern >> shift) ^ (base_pattern << (significant_bits - shift))) & mask
            result = (result * 0x123) & mask
        
        # Apply final transformations
        if position > 20:
            # Additional mixing for later positions
            ...
            if position > 40:
                ...
                result = (result ^ (result >> 3) ^ (position * 0x234567)) & mask
