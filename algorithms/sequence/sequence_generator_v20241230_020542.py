"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:05:42
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Special values for every 5th position
        special_values = {
            5: 0x31,
            10: 0x483,
            15: 0xc936,
        ...
        if position % 5 == 0:
            # For positions divisible by 5, use geometric growth pattern
            result = ['0'] * sig_bits
            
            # First bit is always 1
            result[0] = '1'
            
            # Base pattern derived from position
            base = position // 5
            
            # Pattern 1: Geometric progression with 5-bit groups
            for i in range(0, sig_bits, 5):
                if i < sig_bits:
                    # Set bits in a 5-bit group pattern
                    group = ['0'] * min(5, sig_bits - i)
                    group[0] = '1'  # First bit in group is 1
                    if len(group) > 2:
                        group[2] = '1'  # Middle bit in group is 1
                    if len(group) > 4:
                        group[4] = '1'  # Last bit in group is 1
                    result[i:i+len(group)] = group
            
            # Pattern 2: Position-based mixing
            for i in range(1, sig_bits - 4, 5):
                if i + 4 < sig_bits:
            ...
            # Pattern 3: Ensure minimum hamming weight
            ...
            if weight < min_weight:
                for i in range(0, sig_bits, 5):
                    if weight >= min_weight:
                        break
                    if i + 2 < sig_bits and result[i+2] == '0':
                        result[i+2] = '1'
                        weight += 1
            
            return int(''.join(result), 2)
        
        # For non-5th positions, use 5-bit group pattern
        result = ['0'] * sig_bits
        
        # Pattern 1: Every 5th bit group has a specific pattern
        for i in range(0, sig_bits, 5):
            if i < sig_bits:
                # Set bits in a 5-bit group pattern
                group = ['0'] * min(5, sig_bits - i)
                group[0] = '1'  # First bit in group is 1
                if len(group) > 2 and (position + i) % 3 == 0:
                    group[2] = '1'  # Middle bit in group is 1 based on position
                if len(group) > 4 and (position + i) % 2 == 0:
                    group[4] = '1'  # Last bit in group is 1 based on position
                result[i:i+len(group)] = group
        
        # Pattern 2: Position-based variations
        for i in range(1, sig_bits - 1, 5):
            if i + 1 < sig_bits and (i * position) % 5 == 0:
                result[i] = '1'
                if i + 1 < sig_bits:
                    result[i+1] = '0'  # Ensure alternating pattern
