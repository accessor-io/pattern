"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 01:46:12
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        ...
        if position <= 7:
            ...
            growth = int(math.log2(position - 6) * 2.2)  # Slower growth
            # Add small linear component for consistency
            linear = (position - 7) // 5  # More gradual linear growth
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
            ...
            if position == 8:
                ...
                for i in range(9):
                    if i < 4:
                        ...
                        result[i] = '1' if (i * position + 3) % 4 == 1 else '0'  # Enhanced pattern
            else:
                # Normal case
                preserve_bits = min(sig_bits // 2, 10)  # Keep up to half, max 10
                prev_bits = bits[:preserve_bits]
                # Place preserved bits with enhanced spacing
                for i, bit in enumerate(prev_bits):
                    pos = (i * 2 + position) % sig_bits
                    result[pos] = bit
        
        # Handle middle bits with improved variation
        mid_bits = sig_bits // 2
        mid_start = (position * 4 + 13) % sig_bits
        for i in range(mid_bits):
            src_pos = (i * 3 + position * 2) % len(bits)
        ...
        min_weight = max(sig_bits // 4, position // 4)  # Balanced minimum weight
    ...
    def _apply_non_linear_transform(self, value: int, position: int) -> int:
        ...
        chunks = []
        chunk_count = min(3, sig_bits // 4)  # Fewer chunks for better control
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk = chunk ^ ((chunk << 1) & (chunk >> 1))  # Basic mixing
        ...
        for i, chunk in enumerate(chunks):
            ...
            rotate = (position + i) % (chunk_size // 2)
