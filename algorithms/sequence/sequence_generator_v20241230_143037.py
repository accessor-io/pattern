"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:30:37
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if position <= 15:
            # Early sequence: Mix of shifts and rotations
            shift1 = (position * 2 + 3) % significant_bits
            shift2 = (position + 5) % significant_bits
            result = ((seed << shift1) | (seed >> (significant_bits - shift1))) & mask
            result = result ^ ((seed << shift2) | (seed >> (significant_bits - shift2)))
            result = (result + (position * block_num + 7)) & mask
        elif position <= 30:
            # Middle sequence: More complex transformations
            shift1 = (block_pos * 3 + 4) % significant_bits
            shift2 = (block_num * 2 + 3) % significant_bits
            rotate = (position + 7) % significant_bits
            
            # Mix rotations and XORs
            ...
            result = (result + (position * 13 + block_num * 7)) & mask
            
            # Final rotation
            result = ((result << rotate) | (result >> (significant_bits - rotate))) & mask
        else:
            # Later sequence: Simpler transformations as bits decrease
            shift1 = (block_pos * 2 + 3) % significant_bits
            shift2 = (block_num + 4) % significant_bits
            
            # Mix shifts and XORs
            temp1 = (seed << shift1) & mask
            temp2 = (seed >> shift2) & mask
            result = (temp1 ^ temp2) & mask
            
            # Add small position-based offset
            result = (result + position + block_num) & mask
