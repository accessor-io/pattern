"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:01:41
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        def transform_block(v: int, p: int) -> int:
            ...
            # For later positions, analyze block transitions
            block_num = (p - 2) // 5
            pos_in_block = (p - 2) % 5
            ...
            # Calculate block transition values
            if block_num >= 3:
                ...
                transition = [
                    last_block[i] ^ second_last_block[i]
                    for i in range(5)
                ]
                ...
                for i in range(5):
                    # Mix with transition values
                    base ^= transition[i]
                    # Rotate based on position and block
                    rotate = ((transition[i] + pos_in_block) * (block_num % 7 + 1)) % 11
