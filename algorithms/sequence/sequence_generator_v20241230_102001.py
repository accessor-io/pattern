"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:20:01
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        if position <= 7:
            ...
            return max(32 - (position - 20), 16)  # Decrease from 32 bits
        elif position <= 40:
            return max(16 - (position - 40), 8)   # Decrease from 16 bits
        else:
            return max(8 - ((position - 40) // 2), 4)  # Decrease from 8 bits, but never below 4
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Base transformation
        base_pattern = value & mask
        
        # Apply position-specific transformations
        if block_pos == 0:
            # Every 5th position gets special treatment
            shift = (block_num * 3 + 2) % significant_bits
            result = ((base_pattern << shift) | (base_pattern >> (significant_bits - shift))) & mask
            result = (result + (block_num * 0x12345)) & mask
            
            # Additional mixing for 5th positions
            if position > 20:
                mix = (result ^ (result >> 2)) & mask
                result = (mix + (block_num * 0x67890)) & mask
                
                if position > 40:
                    # Enhanced mixing for later 5th positions
                    mix = (result ^ (result << 3)) & mask
                    result = (mix + (block_num * 0x89abc)) & mask
        elif block_pos == 1:
            # First transition position
            shift = (block_num * 4 + 1) % significant_bits
            result = ((base_pattern >> shift) | (base_pattern << (significant_bits - shift))) & mask
            result = (result ^ (block_num * 0x23456)) & mask
            
            # Enhanced mixing for later positions
            if position > 30:
                mix = (result + (result << 2)) & mask
                result = (mix ^ (block_num * 0x78901)) & mask
                
                if position > 50:
                    # Special mixing for latest positions
                    mix = (result ^ (result >> 3)) & mask
                    result = (mix + (block_num * 0xabcde)) & mask
        elif block_pos == 2:
            # Middle transition position
            shift = (block_num * 5 + 3) % significant_bits
            result = ((base_pattern << shift) ^ (base_pattern >> (significant_bits - shift))) & mask
            result = (result + (block_num * 0x34567)) & mask
            
            # Additional pattern for later positions
            if position > 40:
                mix = (result ^ (result >> 2)) & mask
                result = (mix + (block_num * 0x89012)) & mask
                
                if position > 60:
                    # Special mixing for latest positions
                    mix = (result ^ (result << 4)) & mask
                    result = (mix + (block_num * 0xcdef0)) & mask
        elif block_pos == 3:
            # Late transition position
            shift = (block_num * 2 + 1) % significant_bits
            result = ((base_pattern >> shift) ^ (base_pattern << (significant_bits - shift))) & mask
            result = (result ^ (block_num * 0x45678)) & mask
            
            # Enhanced mixing for later positions
            if position > 50:
                mix = (result + (result << 3)) & mask
                result = (mix ^ (block_num * 0x90123)) & mask
                
                if position > 70:
                    # Special mixing for latest positions
                    mix = (result ^ (result >> 4)) & mask
                    result = (mix + (block_num * 0xef012)) & mask
        else:  # block_pos == 4
            # Pre-special position
            shift = (block_num * 6 + 2) % significant_bits
            result = ((base_pattern << shift) | (base_pattern >> (significant_bits - shift))) & mask
            result = (result + (block_num * 0x56789)) & mask
            
            # Additional pattern for later positions
            if position > 60:
                mix = (result ^ (result >> 3)) & mask
                result = (mix + (block_num * 0xa1234)) & mask
                
                if position > 80:
                    # Special mixing for latest positions
                    mix = (result ^ (result << 5)) & mask
                    result = (mix + (block_num * 0xb2345)) & mask
        
        # Apply final transformations based on position
        if position > 20:
            # Progressive mixing for later positions
            mix_shift = (position * 3) % 7
            result = ((result << mix_shift) | (result >> (significant_bits - mix_shift))) & mask
            
            if position > 40:
                # Enhanced mixing for very late positions
                mix = (result ^ (result >> 2)) & mask
                result = (mix + (position * 0x12345)) & mask
                
                if position > 60:
                    # Special mixing for latest positions
                    mix = (result ^ (result << 3)) & mask
                    result = (mix + (block_num * 0x23456)) & mask
                    
                    if position > 80:
                        # Final mixing for very late positions
                        mix = (result ^ (result >> 4)) & mask
                        result = (mix + (position * 0x34567)) & mask
