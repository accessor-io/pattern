"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:09:11
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        for i in range(min(len(prev_bits), sig_bits)):
            if i < sig_bits:
                if i % 5 == block_pos:
                    # Position-aligned bits follow next value
                    result[i] = next_bits[i] if i < len(next_bits) else '1'
                elif i % 3 == block_pos:
                    # Position-aligned bits follow previous value
                    result[i] = prev_bits[i]
                elif i % 2 == block_pos:
                    # Position-aligned bits alternate
                    result[i] = '1' if (position + i) % 2 == 0 else '0'
                else:
                    # Other bits follow block-based pattern
