"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:23:51
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # For positions after initial values, use precise bit pattern matching
        ...
        # Calculate new value based on bit patterns
        if position < 20:
            # Early positions: focus on bit pattern preservation
            # 1. Get the base pattern by shifting left
            shift = 1 + (position % 2)
            base = prev_value << shift
            
            # 2. Preserve some lower bits from previous value
            mask = (1 << (prev_high_bit // 2)) - 1
            preserved = prev_value & mask
            
            # 3. Add position-dependent variation
            variation = position * 0x111
            
            # Combine components
            new_value = base | preserved | (variation & 0xfff)
        else:
            # Later positions: focus on controlled growth
            # 1. Calculate growth factor
            growth = 1.8 - (position - 20) * 0.02
            growth = max(1.3, growth)
            base = int(prev_value * growth)
            
            # 2. Apply position-specific transformations
            if position % 3 == 0:
                new_value = base ^ (base >> (prev_high_bit // 3))
            elif position % 3 == 1:
                new_value = base | (prev_value & 0xfff)
            else:
                new_value = base ^ (position * 0x111)
        
        # Ensure minimum growth and pattern preservation
        if new_value <= prev_value:
            # If not growing, apply a different transformation
            new_value = prev_value + (prev_value >> 2) + (position * 0x111)
