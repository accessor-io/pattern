"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:58:41
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        def evolve_value(v: int, p: int) -> int:
            ...
            # For later positions, evolve based on previous values
            base = seeds[(p - 2) % len(seeds)]
            
            # Apply position-specific evolution
            result = base
            
            # Mix with previous seed values
            for i in range(3):
                prev_seed = seeds[(p - i - 3) % len(seeds)]
                result ^= prev_seed
                
                # Rotate based on position and seed
                rotate = ((p + prev_seed) % 7) + 1
                result = ((result << rotate) | (result >> (32 - rotate))) & 0xffffffff
                
                # Apply non-linear transform
                result = (result * (0x7b + p)) & 0xffffffff
            
            # Apply position-specific mixing
            result ^= (result >> ((p % 5) + 3)) & ((1 << (28 - p % 7)) - 1)
            
            return result
            
        # Apply the evolution
        x = evolve_value(x, position)
