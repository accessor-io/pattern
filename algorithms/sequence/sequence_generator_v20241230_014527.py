"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 01:45:27
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        ...
        if position <= 7:
            ...
            base_bits = 9
            # Add position-based growth with better scaling
            growth = int(math.log2(position - 6) * 2.5)  # Adjusted growth rate
            # Add small linear component for consistency
            linear = (position - 7) // 4  # More aggressive linear growth
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if position >= 8:
            ...
            if position == 8:
                ...
                # Combine bits with improved pattern
                for i in range(9):
                    if i < 4:
                        result[i] = prev_bits[i]  # Keep first 4 bits
                    elif i < 7:
                        result[i] = target_bits[i]  # Use target bits
                    else:
                        result[i] = '1' if (i * position) % 3 == 1 else '0'  # Enhanced pattern
            else:
                # Normal case
                preserve_bits = min(sig_bits // 2, 12)  # Keep up to half, max 12
                prev_bits = bits[:preserve_bits]
                # Place preserved bits with enhanced spacing
                for i, bit in enumerate(prev_bits):
                    pos = (i * 3 + position) % sig_bits
                    result[pos] = bit
        
        # Handle middle bits with improved variation
        mid_bits = sig_bits // 2
        mid_start = (position * 5 + 11) % sig_bits
        for i in range(mid_bits):
            src_pos = (i * 7 + position * 3) % len(bits)
            dst_pos = (mid_start + i * 3) % sig_bits
            result[dst_pos] = bits[src_pos]
        
        # Ensure minimum hamming weight with better distribution
        result_str = ''.join(result)
        weight = result_str.count('1')
        min_weight = max(sig_bits // 3, position // 3)  # Increased minimum weight
        
        if weight < min_weight:
            # Add bits in a distributed pattern
            gap = max(3, sig_bits // min_weight)
    ...
    def _apply_non_linear_transform(self, value: int, position: int) -> int:
        """Apply non-linear transformation with enhanced mixing"""
        sig_bits = self._get_significant_bits(position)
        
        if position <= 8:
            return value
        
        # Split value into overlapping chunks with better distribution
        chunks = []
        chunk_count = min(4, sig_bits // 4)  # More chunks for better mixing
        chunk_size = sig_bits // chunk_count
        
        for i in range(chunk_count):
            start = (i * chunk_size * 3 // 4) % sig_bits  # 25% overlap
        ...
        for i, chunk in enumerate(chunks):
            ...
            shift1 = ((position + i) % 4) + 2
            shift2 = ((position + i) % 3) + 2
            
            # Multiple rounds with different operations
            chunk = chunk ^ (chunk << shift1)
            chunk = chunk ^ (chunk >> shift2)
            chunk = chunk ^ ((chunk << 2) & (chunk >> 1))  # Enhanced mixing
            chunk = chunk ^ ((chunk >> 3) & (chunk << 2))  # Additional mixing
            chunks[i] = chunk & ((1 << chunk_size) - 1)
        
        # Recombine chunks with enhanced mixing
        result = 0
        for i, chunk in enumerate(chunks):
            shift = (i * chunk_size * 3 // 4) % sig_bits
            result ^= (chunk << shift) & ((1 << sig_bits) - 1)
            # Add rotation for better bit distribution
            rotate = (position + i) % chunk_size
            result = ((result << rotate) | (result >> (sig_bits - rotate))) & ((1 << sig_bits) - 1)
        
        # Final mixing based on position
        mix_shift = (position % 4) + 1
        result ^= (result >> mix_shift) & ((1 << (sig_bits - mix_shift)) - 1)
        result ^= (result << (mix_shift + 1)) & ((1 << sig_bits) - 1)  # Additional mixing
