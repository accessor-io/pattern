"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:21:45
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # For positions after initial values, calculate based on decreasing growth pattern
        ...
        if position == 8:
            ...
            # Calculate growth with decreasing rate
            base_growth = 2.3
            decay = (position - 8) * 0.05  # Decrease by 0.05 each position
            growth = max(1.5, base_growth - decay)  # Don't go below 1.5x
            new_value = int(prev_value * growth)
            
            # Apply position-based mixing to add variation
            if position % 3 == 0:
                new_value = new_value ^ (position * 0x111)
            elif position % 3 == 1:
                new_value = new_value ^ (position * 0x222)
            else:
                new_value = new_value ^ (position * 0x333)
