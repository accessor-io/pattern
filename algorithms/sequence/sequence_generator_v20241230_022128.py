"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:21:28
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Apply bit rotation based on position and block
        rotation = (position * block_pos * 3) % significant_bits
        rotated = ((value << rotation) | (value >> (significant_bits - rotation))) & mask
        
        # Apply additional transformations based on block position
        if block_pos == 1:
            # First position after special value
            rotated = (rotated * 0x1234567) & mask
        elif block_pos == 2:
            # Second position after special value
            rotated = (rotated + 0x789abc) & mask
        elif block_pos == 3:
            # Third position after special value
            rotated = (rotated ^ 0xdef012) & mask
        
        # Apply non-linear transform
        transformed = self._apply_non_linear_transform(rotated, position)
