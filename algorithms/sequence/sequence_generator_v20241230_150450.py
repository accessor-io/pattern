"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:04:50
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        def analyze_block_pattern(values, block_num):
            patterns = []
            for i in range(len(values) - 1):
                # Calculate the transformation between consecutive values
                curr = values[i]
                next_val = values[i + 1]
                ...
                patterns.append({
                    'xor': xor_pattern,
                    'shift': shift_pattern,
                    'multiply': next_val // curr if curr != 0 else 0,
                    'add': next_val - curr
                })
        ...
        def reverse_engineer_value(p: int) -> int:
            ...
            last_complete_block = known_values[-5:]
            patterns = analyze_block_pattern(last_complete_block, block_num - 1)
            
            # Apply pattern based on position
            base = last_complete_block[pos_in_block]
            pattern = patterns[pos_in_block]
