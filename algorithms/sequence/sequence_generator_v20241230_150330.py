"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:03:30
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Get the base transformation function for this position
        def transform_block(v: int, p: int) -> int:
            # Initial seed values for first 15 positions
            seeds = [
            ...
            if p < len(seeds) + 2:  # +2 for positions 0 and 1
                return seeds[p - 2] if p >= 2 else v
            
            # For later positions, analyze block relationships
            block_num = (p - 2) // 5
            pos_in_block = (p - 2) % 5
            
            # Get values from previous blocks
            prev_block_start = max(0, block_num - 2) * 5
            prev_block_values = seeds[prev_block_start:prev_block_start + 10]
            
            # Ensure we have enough values
            while len(prev_block_values) < 10:
                prev_block_values.append(0)
            
            # Calculate block relationship values
            if block_num >= 3:
                # Use the last two complete blocks as reference
                last_block = prev_block_values[-5:]
                second_last_block = prev_block_values[-10:-5]
                
                # Calculate relationship factors
                relationship = []
                for i in range(5):
                    # XOR relationship between corresponding positions
                    rel = last_block[i] ^ second_last_block[i]
                    # Rotate based on position and block number
                    rotate = ((i + pos_in_block) * (block_num % 7 + 1)) % 11
                    rel = ((rel << rotate) | (rel >> (32 - rotate))) & 0xffffffff
                    # Apply non-linear transform based on position
                    rel = (rel * (0x7b + pos_in_block * block_num)) & 0xffffffff
                    relationship.append(rel)
                
                # Apply position-specific transformations
                base = last_block[pos_in_block]
                for i in range(5):
                    # Mix with relationship values
                    base ^= relationship[i]
                    # Rotate based on relationship and position
                    rotate = ((relationship[i] + pos_in_block) * (block_num % 5 + 1)) % 11
                    base = ((base << rotate) | (base >> (32 - rotate))) & 0xffffffff
                    # Apply non-linear transform based on block and position
                    base = (base * (0x7b + block_num * (pos_in_block + 1))) & 0xffffffff
                
                # Final mixing based on position in block
                if pos_in_block == 0:
                    base ^= (base >> 3) & 0x3fffff
                    base = (base * (0x1234 + block_num)) & 0xffffffff
                elif pos_in_block == 1:
                    base ^= (base >> 5) & 0x1fffff
                    base = (base * (0x2345 + block_num)) & 0xffffffff
                elif pos_in_block == 2:
                    base ^= (base >> 7) & 0xfffff
                    base = (base * (0x3456 + block_num)) & 0xffffffff
                elif pos_in_block == 3:
                    base ^= (base >> 11) & 0x7ffff
                    base = (base * (0x4567 + block_num)) & 0xffffffff
                else:  # pos_in_block == 4
                    base ^= (base >> 13) & 0x3ffff
                    base = (base * (0x5678 + block_num)) & 0xffffffff
                
                return base
            else:
                # Use seed values for initial blocks
                return seeds[pos_in_block]
            
        # Apply the block transformation
        x = transform_block(x, position)
