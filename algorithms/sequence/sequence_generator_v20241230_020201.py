"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:02:01
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if position % 5 == 0:
            ...
            # For other multiples of 5, use binary pattern
            # Pattern: XOR-based groups with alternating patterns
            result = ['0'] * sig_bits
            
            # Pattern 1: First bit is always 1
            result[0] = '1'
            
            # Pattern 2: Groups of 2-3 bits with XOR patterns
            for i in range(1, sig_bits - 2, 3):
                if i + 2 < sig_bits:
                    # First bit in group is XOR of previous two bits
                    prev_bits = int(''.join(result[max(0, i-2):i]), 2) if i > 1 else 1
                    result[i] = '1' if bin(prev_bits).count('1') % 2 == 0 else '0'
                    
                    # Second bit alternates based on position
                    result[i+1] = '1' if ((i + position) // 3) % 2 == 0 else '0'
                    
                    # Third bit is XOR of first two bits in group
                    group_bits = int(''.join(result[i:i+2]), 2)
                    result[i+2] = '1' if bin(group_bits).count('1') % 2 == 0 else '0'
            
            # Pattern 3: Position-based variations
            base = position // 5
            for i in range(2, sig_bits, base + 2):
                if i < sig_bits:
                    # XOR with adjacent bits
                    prev_bits = int(''.join(result[max(0, i-2):i]), 2)
                    result[i] = '1' if bin(prev_bits).count('1') % 2 == 0 else '0'
            
            # Pattern 4: Ensure minimum hamming weight
            weight = ''.join(result).count('1')
            min_weight = max(3, sig_bits // 3)
        ...
        # For non-5th positions, use regular pattern
        result = ['0'] * sig_bits
        
        # Pattern 1: Every 3rd bit is 1
        for i in range(0, sig_bits, 3):
            result[i] = '1'
        
        # Pattern 2: Position-based variations
        for i in range(1, sig_bits, 2):
            if (i * position) % 4 == 0:
                result[i] = '1'
