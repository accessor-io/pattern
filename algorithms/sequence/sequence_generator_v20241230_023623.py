"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:36:23
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if (position - 4) % 5 == 0:  # positions 9, 14, 19, 24, 29, etc.
            ...
            if prev_special and next_special:
                ...
                # Calculate weighted ratio based on position
                ratio = (position - prev_special) / (next_special - prev_special)
                # Apply non-linear weighting for smoother transitions
                weighted_ratio = (1 - math.cos(ratio * math.pi)) / 2
                
                # Extract significant bits patterns from special values
                prev_pattern = prev_value & ((1 << significant_bits) - 1)
                next_pattern = next_value & ((1 << significant_bits) - 1)
                
                # Interpolate between patterns with enhanced mixing
                base_interpolated = int(prev_pattern * (1 - weighted_ratio) + next_pattern * weighted_ratio)
                
                # Apply position-specific transformations
                shift_amount = (position % 5) * 3
                rotated = ((base_interpolated << shift_amount) | (base_interpolated >> (significant_bits - shift_amount)))
                
                # Add position-specific pattern
                pattern_mix = (position * 0x1234567) & ((1 << significant_bits) - 1)
                result = (rotated ^ pattern_mix) & ((1 << significant_bits) - 1)
