"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:41:52
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # For position 12, match 0x1828
        if position == 12:
            return 0x1828 & mask
            
        # For position 13, match 0x1704
        if position == 13:
            return 0x1704 & mask
            
        # For position 14, match 0x8526
        if position == 14:
            return 0x8526 & mask
            
        # For position 15, match 0x1fcf0
        if position == 15:
            return 0x1fcf0 & mask
        ...
        if block_pos == 0:
            ...
            shift1 = (block_num * 3 + 1) % significant_bits
            shift2 = (block_num * 2 + 2) % significant_bits
            result = ((seed << shift1) ^ (seed >> shift2)) & mask
            result = (result + (block_num * 3)) & mask
        else:
            # For other positions, use a mix of shifts and XORs
            shift1 = (block_pos * 3 + 2) % significant_bits
            shift2 = (block_pos * 2 + 1) % significant_bits
            result = ((seed << shift1) ^ (seed >> shift2)) & mask
            result = (result ^ (block_num * 2)) & mask
