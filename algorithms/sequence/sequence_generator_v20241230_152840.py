"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:28:40
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        def highest_bit_pos(n):
            return max(1, len(bin(n)) - 3)  # Ensure minimum of 1 bit
        
        prev_high_bit = highest_bit_pos(prev_value)
        
        # Calculate growth based on position and previous value
        growth_factor = 1.0 + (position / (prev_high_bit + 1))
        base = int(prev_value * growth_factor)
        
        # Apply bit rotations based on position
        rotation = position % (prev_high_bit + 1)  # Add 1 to avoid division by zero
        rotated = ((base << rotation) | (base >> (prev_high_bit - rotation + 1))) & ((1 << (prev_high_bit * 2)) - 1)
        
        # Apply non-linear transformations
        pattern = (position * 0x1111) & ((1 << prev_high_bit) - 1)
        transformed = rotated ^ pattern
        
        # Ensure growth
        if transformed <= prev_value:
            transformed = prev_value + (position + 1)
