...
def plot_sliding_window_xor(output_grid, window_size):
    plt.figure(figsize=(12, 8))
    sns.heatmap(output_grid, cmap='coolwarm', annot=False)
    plt.title(f'Sliding Window XOR ({window_size}x{window_size}) of Horizontal and Vertical Grids')
    plt.xlabel('Position')
    plt.ylabel('Level')
    plt.tight_layout()
    plt.savefig(f'multiscale_sliding_window_xor_{window_size}x{window_size}.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    for window_size in [2, 3, 4]:
        ...
        plot_sliding_window_xor(output_grid, window_size) 