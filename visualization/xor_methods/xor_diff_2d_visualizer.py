...
def compute_2d_xor_grids(table):
    # Horizontal XORs (already in table, but let's make a grid for visualization)
    max_len = len(table[0])
...
def plot_2d_xor_grids(horizontal_grid, vertical_grid):
    fig, axs = plt.subplots(2, 1, figsize=(18, 14))
    sns.heatmap(horizontal_grid, ax=axs[0], cmap='viridis', annot=False, fmt='.0f')
    axs[0].set_title('Horizontal XOR Difference Table')
    axs[0].set_xlabel('Position')
    axs[0].set_ylabel('Level')
    sns.heatmap(vertical_grid, ax=axs[1], cmap='magma', annot=False, fmt='.0f')
    axs[1].set_title('Vertical XOR Differences (between levels)')
    axs[1].set_xlabel('Position')
    axs[1].set_ylabel('Level')
    plt.tight_layout()
    plt.savefig('xor_diff_2d_visualization.png')
    plt.close()

if __name__ == "__main__":
    table = build_difference_table()
    horizontal_grid, vertical_grid = compute_2d_xor_grids(table)
    plot_2d_xor_grids(horizontal_grid, vertical_grid) 