"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:34:33
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Calculate base value using dynamic growth patterns
        base = prev_value
        if position > 8:
            # Apply position-based transformations
            shift = (position * prev_high_bit) % 32
            pattern = (position * 0x1111) & ((1 << prev_high_bit) - 1)
            
            # Use multiple transformations to create complex patterns
            t1 = ((base << shift) | (base >> (32 - shift))) & ((1 << (prev_high_bit * 2)) - 1)
            t2 = t1 ^ pattern
            t3 = t2 | (t2 >> (prev_high_bit // 2))
            
            # Apply non-linear growth
            if position % 3 == 0:
                base = t3 + (t3 >> (prev_high_bit // 3))
            elif position % 3 == 1:
                base = t3 ^ (t3 << (prev_high_bit // 4))
            else:
                base = t3 | (position * 0x11111111)
        
        # Ensure the value grows
        if base <= prev_value:
            base = prev_value + (position * 0x111)
