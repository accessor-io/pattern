"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:55:44
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if pos_mod == 0:
            # First position in block (0x8, 0x202, etc.)
            x = (x << 3) | (x >> 29)
            x = x | 0x8  # Set bit 3
            x = x | ((position >> 3) << 9)  # Higher bits based on block
            
        elif pos_mod == 1:
            # Second position in block (0x15, 0x483, etc.)
            x = (x << 2) | (x >> 30)
            x = x | 0x15  # Set bits 0,2,4
            x = x | ((position >> 3) << 10)  # Higher bits based on block
            
        elif pos_mod == 2:
            # Third position in block (0x31, 0xa7b, etc.)
            x = (x << 1) | (x >> 31)
            x = x | 0x31  # Set bits 0,4,5
            x = x | ((position >> 3) << 11)  # Higher bits based on block
            
        elif pos_mod == 3:
            # Fourth position in block (0x4c, 0x1460, etc.)
            x = (x << 4) | (x >> 28)
            x = x | 0x4c  # Set bits 2,3,6
            x = x | ((position >> 3) << 12)  # Higher bits based on block
            
        else:
            # Fifth position in block (0xe0, 0x2930, etc.)
            x = (x << 3) | (x >> 29)
            x = x | 0xe0  # Set bits 5,6,7
            x = x | ((position >> 3) << 13)  # Higher bits based on block
