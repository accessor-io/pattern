"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:18:29
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # For positions after initial values, try to follow the pattern from original sequence
        if position >= 8:
            # Get the last two values
            prev_value = self.sequence[-1]
            second_last_value = self.sequence[-2]
            
            # Calculate the relationship between the last two values
            relationship = prev_value ^ second_last_value
            
            # Apply a position-based transformation
            new_value = prev_value ^ (relationship << (position % 4))
            
            # Mix in some bits from earlier in the sequence
            if len(self.sequence) >= 5:
                five_back = self.sequence[-5]
                new_value ^= five_back >> (position % 3)
