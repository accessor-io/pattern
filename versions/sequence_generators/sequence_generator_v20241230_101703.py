"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:17:03
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        if position <= 7:
            ...
            return min(32, position + 2)
        elif position <= 40:
            return min(48, position + 3)
        else:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if block_pos == 0:
            ...
            shift = (block_num * 7 + 3) % significant_bits
            ...
            shift = (block_num * 5 + 2) % significant_bits
            result = ((base_pattern >> shift) | (base_pattern << (significant_bits - shift))) & mask
            ...
            shift = (block_num * 3 + 1) % significant_bits
            result = ((base_pattern << shift) ^ (base_pattern >> (significant_bits - shift))) & mask
            ...
            shift = (block_num * 4 + 2) % significant_bits
            result = ((base_pattern >> shift) ^ (base_pattern << (significant_bits - shift))) & mask
            result = (result ^ (block_num * 0x78901)) & mask
            ...
            shift = (block_num * 6 + 1) % significant_bits
            result = ((base_pattern << shift) | (base_pattern >> (significant_bits - shift))) & mask
            result = (result + (block_num * 0x90123)) & mask
