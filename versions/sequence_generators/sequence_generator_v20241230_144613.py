"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:46:13
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if block_pos == 0:  # First position in block
            shift = (block_num * 2 + 1) % 32
            temp = (value << shift) | (value >> (32 - shift))
            result = temp + block_num
            
        elif block_pos == 1:  # Second position
            shift = (block_num * 3 + 2) % 32
            temp = (value << shift) | (value >> (32 - shift))
            result = temp ^ (block_num * 0x11)
            
        elif block_pos == 2:  # Middle position
            shift1 = (block_num * 5 + 3) % 32
            shift2 = (block_num * 7 + 4) % 32
            temp1 = (value << shift1) | (value >> (32 - shift1))
            temp2 = (value << shift2) | (value >> (32 - shift2))
            result = temp1 + temp2
            
        elif block_pos == 3:  # Fourth position
            rotate = (block_num * 11 + 5) % 32
            temp = (value << rotate) | (value >> (32 - rotate))
            result = temp ^ (block_num * 0x101)
            
        else:  # Last position in block
            shift = (block_num * 13 + 6) % 32
            temp = (value << shift) | (value >> (32 - shift))
            result = temp + (block_num * 0x202)
