"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:45:03
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Use 32-bit width but with controlled growth
        significant_bits = min(32, 8 + block_pos * 2)
        mask = (1 << significant_bits) - 1
        seed = value & mask
        
        # Base transformation based on block position
        if block_pos == 0:  # First position in block
            shift = (block_num * 2 + 1) % significant_bits
            temp = ((seed << shift) | (seed >> (significant_bits - shift))) & mask
            result = (temp + block_num) & mask
            
        elif block_pos == 1:  # Second position
            shift = (block_num * 3 + 2) % significant_bits
            temp = ((seed << shift) | (seed >> (significant_bits - shift))) & mask
            result = (temp ^ (block_num * 0x11)) & mask
            
        elif block_pos == 2:  # Middle position
            shift1 = (block_num * 5 + 3) % significant_bits
            shift2 = (block_num * 7 + 4) % significant_bits
        ...
        elif block_pos == 3:  # Fourth position
            rotate = (block_num * 11 + 5) % significant_bits
            temp = ((seed << rotate) | (seed >> (significant_bits - rotate))) & mask
            result = (temp ^ (block_num * 0x101)) & mask
            
        else:  # Last position in block
            shift = (block_num * 13 + 6) % significant_bits
            temp = ((seed << shift) | (seed >> (significant_bits - shift))) & mask
            result = (temp + (block_num * 0x202)) & mask
        
        # Final adjustment based on position
        result = (result + position) & mask
        return result
