"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:40:28
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value, position):
        ...
        for i in range(min(4, position - 7)):  # Limit transformations for controlled growth
            # Extract chunks of bits
            chunk = (base >> (i * 4)) & 0xf
            # Mix bits using position-dependent shifts
            shift = ((position + i) * chunk) % 16
            # Combine with new value
            new_value = (new_value << shift) | (new_value >> (64 - shift)) if shift else new_value
            new_value |= (chunk << (i * 8))
            
        # Ensure we don't exceed reasonable bounds
        max_bits = min(prev_bits + 4, 64)  # Allow gradual growth
        mask = (1 << max_bits) - 1
