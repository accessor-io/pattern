"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:41:25
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value, position):
        ...
        # with a specific bit manipulation pattern that grows over time
        base = value
        new_value = value
        
        # Apply transformations based on position
        for i in range(min(2, position - 7)):  # Limit transformations for controlled growth
            # Extract chunks of bits
            chunk = (base >> (i * 4)) & 0xf
            # Mix bits using position-dependent shifts
            shift = ((position + i) * chunk) % 4
            # Combine with new value
            new_value = (new_value << shift) | (new_value >> (16 - shift)) if shift else new_value
            new_value ^= (chunk << (i * 2))
            
        # Ensure we don't exceed reasonable bounds
        max_bits = min(prev_bits + 1, 16)  # Allow very gradual growth
        mask = (1 << max_bits) - 1
