"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:10:47
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Pattern 1: Base pattern from previous special value
        ...
        # Mix bits based on position
        for i in range(min(len(prev_bits), sig_bits)):
            if i < sig_bits:
                if block_pos == 0:
                    # Every 5th position follows special pattern
                    if i % 3 == 0:
                        result[i] = next_bits[i] if i < len(next_bits) else '1'
                    elif i % 2 == 0:
                        result[i] = prev_bits[i]
                    else:
                        result[i] = '1' if (block_num + i) % 2 == 0 else '0'
                else:
                    # Positions between 5th positions use transition pattern
                    if i % 5 == block_pos:
                        result[i] = next_bits[i] if i < len(next_bits) else '1'
                    elif i % 3 == block_pos:
                        result[i] = prev_bits[i]
                        ...
                        result[i] = '1' if ((block_num + i) * block_pos) % 3 == 0 else '0'
        
        # Pattern 2: Ensure minimum hamming weight
        weight = ''.join(result).count('1')
        min_weight = max(3, sig_bits // 4)
        
        if weight < min_weight:
            for i in range(block_pos, sig_bits, 5):
                if weight >= min_weight:
                    break
                if result[i] == '0':
                    result[i] = '1'
        ...
        # Pattern 3: Position-based mixing
        for i in range(block_pos, sig_bits - 4, 5):
            if i + 4 < sig_bits:
                # Mix with previous group
                prev_group = result[max(0, i-5):i]
                ones = prev_group.count('1')
                if block_pos == 0:
                    # Special mixing for 5th positions
                    result[i+1] = '1' if ones % 3 == 0 else '0'
                    result[i+3] = '1' if ones % 2 == 0 else '0'
                else:
                    # Normal mixing for other positions
                    result[i+1] = '1' if ones % 2 == 0 else '0'
                    result[i+3] = '1' if ones % 3 == 0 else '0'
        
        # Pattern 4: Block-based adjustments
        if block_pos > 0:
            # Ensure smooth transition between blocks
            for i in range(0, sig_bits - block_pos, 5):
                if i + block_pos < sig_bits:
                    # Mix bits based on block position
                    prev_group = result[max(0, i-5):i]
                    next_group = result[i:i+5]
                    if len(prev_group) > 0 and len(next_group) > block_pos:
                        # Adjust bits based on surrounding context
                        if prev_group.count('1') > next_group.count('1'):
                            result[i+block_pos] = '1'
                        elif prev_group.count('1') < next_group.count('1'):
                            result[i+block_pos] = '0'
        
        # Pattern 5: Growth pattern adjustments
        if block_pos > 0:
            # Apply growth pattern based on block number
            growth_bits = block_num * 5
            for i in range(max(0, sig_bits - growth_bits), sig_bits):
                if i % 5 == block_pos:
                    result[i] = '1' if (block_num + i) % 2 == 0 else '0'
