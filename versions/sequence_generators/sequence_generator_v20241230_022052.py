"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:20:52
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if (position - 4) % 5 == 0:  # positions 9, 14, 19, 24, 29, etc.
            ...
            if position == 9:
                ...
                return 0x122fca143c05
            elif position == 49:
                return 0x22bd43c2e9354
            elif position == 54:
                return 0x6abe1f9b67e114
            elif position == 59:
                return 0xfc07a1825367bbe
            elif position == 64:
                return 0x1a838b13505b26867
            
            # For positions beyond 64, apply a pattern based on previous values
            pattern_base = (position // 5) * 0x1234567
            return (base_value + pattern_base) & ((1 << significant_bits) - 1)
            
        # For other positions, apply standard bit permutation
        significant_bits = self._get_significant_bits(position)
        mask = (1 << significant_bits) - 1
        
        # Apply bit rotation based on position
        rotation = (position * 3) % significant_bits
