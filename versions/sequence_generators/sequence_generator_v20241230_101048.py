"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:10:48
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        section_size = max(2, significant_bits // 4)
        num_sections = max(1, significant_bits // section_size)
        ...
        for i in range(num_sections):
            ...
            rotate = ((position + i) % (section_size - 1)) + 1
            section = ((section << rotate) | (section >> (section_size - rotate))) & ((1 << section_size) - 1)
            
            # Apply position-based mixing
            if i > 0:
                prev_section = (transformed >> ((i-1) * section_size)) & ((1 << section_size) - 1)
                section ^= prev_section
            
            # Store transformed section
    ...
    def _apply_non_linear_transform(self, value: int, position: int) -> int:
        ...
        chunk_size = max(2, sig_bits // 3)
        num_chunks = max(1, sig_bits // chunk_size)
        ...
        for i in range(num_chunks):
            ...
            # Apply position-based mixing
            if i > 0:
                prev_chunk = (transformed >> ((i-1) * chunk_size)) & ((1 << chunk_size) - 1)
                chunk ^= prev_chunk
            
            # Store transformed chunk
