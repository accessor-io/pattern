"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:48:28
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Simple bit operations based on position
        if pos_in_block == 0:
            # First position: Simple rotation
            x = (x << 3) | (x >> 29)
            
        elif pos_in_block == 1:
            # Second position: Shift and add
            x = (x << 1) + block
            
        elif pos_in_block == 2:
            # Middle position: XOR with block
            x = x ^ block
            
        elif pos_in_block == 3:
            # Fourth position: Rotate right
            x = (x >> 2) | (x << 30)
            
        else:
            # Last position: Combine operations
            x = ((x << 1) + block) ^ (x >> 1)
