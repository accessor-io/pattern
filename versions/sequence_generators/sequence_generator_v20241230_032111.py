"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 03:21:11
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        for i in range(len(bits)):
            if i % 2 == 0:
                # Even positions: rotate left
                shift = (position + i) % 64
                result |= ((value >> i) & 1) << ((i + shift) % 64)
            else:
                # Odd positions: rotate right
                shift = (position - i) % 64
                result |= ((value >> i) & 1) << ((i - shift) % 64)
        
        # Apply additional transformations based on position ranges
        if position < 10:
            ...
            result ^= position * 0x44
        elif position < 50:
            result ^= position * 0x55
        else:
            result ^= position * 0x66
