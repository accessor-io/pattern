"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:05:30
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        def analyze_block_pattern(values):
            patterns = []
            for i in range(len(values) - 1):
                curr = values[i]
                next_val = values[i + 1]
                
                # Find the most likely transformation pattern
                xor_pattern = curr ^ next_val
                shift_pattern = 0
                ...
                multiplier = 1
                if curr != 0:
                    # Try to find a multiplier that gets us closer to next_val
                    for m in range(1, 16):
        ...
        def reverse_engineer_value(p: int) -> int:
            ...
            # Get the last known complete block
            last_block_start = ((len(known_values) - 1) // 5) * 5
            last_complete_block = known_values[last_block_start:last_block_start + 5]
            
            # Analyze patterns between blocks
            patterns = analyze_block_pattern(last_complete_block)
            
            # Use pattern from corresponding position
            pattern_idx = min(pos_in_block, len(patterns) - 1)
            pattern = patterns[pattern_idx]
            
            # Start with the value from the same position in the last known block
            base = last_complete_block[pos_in_block % len(last_complete_block)]
            
            # Apply block-specific modifications first
            if block_num % 3 == 0:
            ...
            if pattern['shift'] > 0:
                base = ((base >> pattern['shift']) | (base << (32 - pattern['shift']))) & 0xffffffff
            elif pattern['shift'] < 0:
                shift = -pattern['shift']
            ...
            # Apply multiplier if it seems significant
            if pattern['multiply'] > 1:
                base = (base * pattern['multiply']) & 0xffffffff
            
            return base
        
        # Apply reverse engineering
        x = reverse_engineer_value(position)
