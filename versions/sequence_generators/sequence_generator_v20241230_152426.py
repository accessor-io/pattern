"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:24:26
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # For positions after initial values, use structured growth pattern
        ...
        # Calculate new value based on structured patterns
        if position < 25:
            # Early positions: focus on structured growth
            # 1. Calculate base growth
            growth = 1.7 + (position % 3) * 0.1  # Varies between 1.7x and 1.9x
            base = int(prev_value * growth)
            
            # 2. Apply position-dependent transformations
            if position % 4 == 0:
                # Mix high and low bits
                new_value = base ^ (base >> (prev_high_bit // 3))
            elif position % 4 == 1:
                # Preserve structure from previous value
                mask = (1 << (prev_high_bit // 2)) - 1
                new_value = (base & ~mask) | (prev_value & mask)
            elif position % 4 == 2:
                # Add controlled variation
                new_value = base ^ ((position * 0x111) & 0xfff)
            else:
                # Combine patterns
                new_value = base | (prev_value & 0x7ff)
        else:
            # Later positions: more controlled growth
            # 1. Calculate base value
            growth = 1.5 - (position - 25) * 0.01
            growth = max(1.2, growth)
            base = int(prev_value * growth)
            
            # 2. Apply structured transformations
            if position % 3 == 0:
                new_value = base ^ (base >> (prev_high_bit // 4))
            elif position % 3 == 1:
                new_value = base | (prev_value & 0xfff)
            else:
                new_value = base ^ ((position * 0x111) & 0x7fff)
        
        # Ensure minimum growth and pattern preservation
        if new_value <= prev_value:
            # If not growing, apply a different transformation
            new_value = prev_value + (prev_value >> 3) + ((position * 0x111) & 0xfff)
