"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 01:37:23
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        ...
        if position <= 7:
            ...
            # Start with base of 9 bits (next after position 7)
            base_bits = 9
            # Add logarithmic growth based on position
            growth = int(math.log2(position - 6) * 3)  # Slower growth
            # Add small linear component
            linear = (position - 7) // 4
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if position >= 8:
            ...
            # Place preserved bits with spacing
            spacing = max(1, sig_bits // preserve_bits)
            for i, bit in enumerate(prev_bits):
                result[i * spacing % sig_bits] = bit
        
        # Handle middle bits with position-based variation
        mid_bits = sig_bits // 2
        mid_start = (position * 7 + 13) % sig_bits
        for i in range(mid_bits):
            src_pos = (i * 11 + position * 3) % len(bits)
        ...
        min_weight = max(sig_bits // 3, position // 2)
    ...
    def _apply_non_linear_transform(self, value: int, position: int) -> int:
        ...
        chunks = []
        chunk_count = min(5, sig_bits // 4)
        chunk_size = sig_bits // chunk_count
        
        for i in range(chunk_count):
            start = (i * chunk_size * 2 // 3) % sig_bits  # 33% overlap
        ...
        for i, chunk in enumerate(chunks):
            ...
            shift1 = ((position + i) % 5) + 2
            shift2 = ((position + i) % 3) + 3
