"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:50:35
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Block-based bit patterns
        if pos_in_block == 0:
            # First position: Initialize block pattern
            x = (x << 3) | (x >> 29)
            x = x + ((block * 0x8) & 0xFF)
            
        elif pos_in_block == 1:
            # Second position: Build block pattern
            x = (x << 2) | (x >> 30)
            x = x + ((block * 0x10) & 0xFF)
            
        elif pos_in_block == 2:
            # Middle position: Peak block pattern
            x = (x << 4) | (x >> 28)
            x = x + ((block * 0x20) & 0xFF)
            
        elif pos_in_block == 3:
            # Fourth position: Descend block pattern
            x = (x << 1) | (x >> 31)
            x = x + ((block * 0x10) & 0xFF)
            
        else:
            # Last position: Reset block pattern
            x = (x << 3) | (x >> 29)
            x = x + ((block * 0x8) & 0xFF)
        
        # Final adjustment based on block and position
        x = x + (position & 0xF)
