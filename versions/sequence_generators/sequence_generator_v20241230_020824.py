"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:08:24
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        for i in range(min(len(prev_bits), sig_bits)):
            if i < sig_bits:
                if i % 5 == 0:
                    # Every 5th bit follows next value
                    result[i] = next_bits[i] if i < len(next_bits) else '1'
                elif i % 3 == 0:
                    # Every 3rd bit follows previous value
                    result[i] = prev_bits[i]
                elif i % 2 == 0:
                    # Every 2nd bit alternates
                    result[i] = '1' if (position + i) % 2 == 0 else '0'
                else:
                    # Other bits follow position-based pattern
                    result[i] = '1' if (position * i) % 3 == 0 else '0'
