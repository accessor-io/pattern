"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:49:41
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if pos_in_block == 0:
            # First position: Initialize block
            x = (x << 3) | (x >> 29)
            x = x + (block * 7)
            
        elif pos_in_block == 1:
            # Second position: Build sequence
            x = (x << 2) | (x >> 30)
            x = x + (block * 11)
            
        elif pos_in_block == 2:
            # Middle position: Peak transformation
            x = (x << 4) | (x >> 28)
            x = x + (block * 13)
            
        elif pos_in_block == 3:
            # Fourth position: Descend transformation
            x = (x << 1) | (x >> 31)
            x = x + (block * 17)
            
        else:
            # Last position: Reset transformation
            x = (x << 3) | (x >> 29)
            x = x + (block * 19)
