"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:31:48
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        if position <= 7:
            ...
            # Maintain full 32-bit width for all positions after initial sequence
            return 32
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        block_pos = position % 8
        block_num = position // 8
        
        # Apply transformations based on position range
        if position <= 15:
            # Early sequence: Controlled growth with specific bit patterns
            shift1 = (position * 3 + 2) % significant_bits
            shift2 = (position * 2 + 1) % significant_bits
            rotate = (position + 3) % significant_bits
            
            # Mix rotations with controlled growth
            temp1 = ((seed << shift1) | (seed >> (significant_bits - shift1))) & mask
            temp2 = ((seed << shift2) | (seed >> (significant_bits - shift2))) & mask
            result = (temp1 ^ temp2) & mask
            
            # Add position-based offset
            result = (result + (position * 5 + block_num * 3)) & mask
            
            # Final rotation
            result = ((result << rotate) | (result >> (significant_bits - rotate))) & mask
            
        elif position <= 30:
            # Middle sequence: Complex transformations with full bit width
            shift1 = (block_pos * 4 + 3) % significant_bits
            shift2 = (block_num * 3 + 2) % significant_bits
            rotate = (position + 5) % significant_bits
            
            # Mix multiple transformations
            temp1 = ((seed << shift1) | (seed >> (significant_bits - shift1))) & mask
            temp2 = ((seed << shift2) | (seed >> (significant_bits - shift2))) & mask
            result = (temp1 ^ temp2) & mask
            
            # Add block-based offset
            result = (result + (block_pos * 7 + block_num * 11)) & mask
            
            # Additional transformation based on position
            temp3 = ((result << rotate) | (result >> (significant_bits - rotate))) & mask
            result = (result ^ temp3) & mask
            
        else:
            # Later sequence: Controlled decrease with simpler patterns
            shift1 = (block_pos * 2 + 1) % significant_bits
            shift2 = (block_num + 2) % significant_bits
            rotate = position % significant_bits
            
            # Mix shifts and rotations
            temp1 = ((seed << shift1) | (seed >> (significant_bits - shift1))) & mask
            temp2 = (seed >> shift2) & mask
            result = (temp1 ^ temp2) & mask
            
            # Add small position-based offset
            result = (result + (position + block_num * 2)) & mask
            
            # Final rotation for pattern maintenance
            result = ((result << rotate) | (result >> (significant_bits - rotate))) & mask
