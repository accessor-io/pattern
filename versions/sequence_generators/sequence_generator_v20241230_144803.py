"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:48:03
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if pos_in_block == 0:
            # First position: Set base pattern
            x = (x << 3) | (x >> 29)  # Rotate left by 3
            x = x + (block * 8)  # Add block offset
            
        elif pos_in_block == 1:
            # Second position: Increment pattern
            x = (x << 1) | (x >> 31)  # Rotate left by 1
            x = x + (block * 16 + 5)  # Add block offset plus constant
            
        elif pos_in_block == 2:
            # Middle position: Peak
            x = (x << 2) | (x >> 30)  # Rotate left by 2
            x = x + (block * 32 + 8)  # Add block offset plus constant
            
        elif pos_in_block == 3:
            # Fourth position: Descend
            x = (x << 4) | (x >> 28)  # Rotate left by 4
            x = x + (block * 16 + 13)  # Add block offset plus constant
            
        else:
            # Last position: Reset for next block
            x = (x << 3) | (x >> 29)  # Rotate left by 3
            x = x + (block * 8 + 21)  # Add block offset plus constant
