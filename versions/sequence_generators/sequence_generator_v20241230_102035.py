"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:20:35
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        if position <= 7:
            ...
            return max(8 - ((position - 40) // 4), 4)  # Decrease more slowly, never below 4
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if block_pos == 0:
            ...
            if position > 30:
                mix = (result + (result << 1)) & mask
                result = (mix ^ (block_num * 0x789)) & mask
        elif block_pos == 2:
            ...
            if position > 40:
                mix = (result ^ (result >> 1)) & mask
                result = (mix + (block_num * 0x890)) & mask
        elif block_pos == 3:
            ...
            if position > 50:
                mix = (result + (result << 1)) & mask
                result = (mix ^ (block_num * 0x901)) & mask
        else:  # block_pos == 4
            ...
            if position > 60:
                mix = (result ^ (result >> 1)) & mask
                result = (mix + (block_num * 0xa12)) & mask
        ...
        if position > 20:
            ...
            if position > 40:
                ...
                mix = (result ^ (result >> 1)) & mask
                result = (mix + (position * 0x123)) & mask
