"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:02:51
        ...
        def transform_block(v: int, p: int) -> int:
            ...
            if block_num >= 3:
                ...
                for i in range(5):
                    ...
                    rotate = (i + pos_in_block) % 11
                    rel = ((rel << rotate) | (rel >> (32 - rotate))) & 0xffffffff
                    # Apply non-linear transform
                    rel = (rel * (0x7b + block_num * pos_in_block)) & 0xffffffff
                ...
                for i in range(5):
                    ...
                    base ^= relationship[i]
                    # Rotate based on relationship
                    rotate = ((relationship[i] + pos_in_block) * (block_num % 7 + 1)) % 11
                    base = ((base << rotate) | (base >> (32 - rotate))) & 0xffffffff
                    # Apply non-linear transform
                    base = (base * (0x7b + block_num * pos_in_block)) & 0xffffffff
                
                # Final mixing
                base ^= (base >> (pos_in_block + 3)) & 0x3fffff
