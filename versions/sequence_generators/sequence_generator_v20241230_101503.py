"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:15:03
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        ...
        # Base significant bits calculation with smoother transitions
        if position < 10:
            return 8 + ((position - 7) * 2) // 3
        elif position < 20:
            return 10 + ((position - 9) * 3) // 4
        elif position < 30:
            return 18 + ((position - 19) * 4) // 5
        elif position < 40:
            return 26 + ((position - 29) * 5) // 6
        else:
            return 34 + ((position - 39) * 6) // 7
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        # Dynamic chunk size based on position and significant bits
        if position < 10:
            base_chunk = 2
        elif position < 20:
            base_chunk = 3
        ...
        num_chunks = (significant_bits + base_chunk - 1) // base_chunk
        chunk_sizes = []
        remaining_bits = significant_bits
        
        for i in range(num_chunks):
            if i == num_chunks - 1:
                chunk_sizes.append(remaining_bits)
            else:
                # Variable chunk sizes based on position
                size = min(base_chunk + (position % 2), remaining_bits)
                chunk_sizes.append(size)
                remaining_bits -= size
        
        # Transform each chunk while preserving relationships
        transformed = 0
        prev_chunk = 0
        current_bit = 0
        ...
        for i, chunk_size in enumerate(chunk_sizes):
            ...
            chunk = (result >> current_bit) & chunk_mask
            
            # Apply position-based transformation
            if i == 0:
                # First chunk preserves more original bits
                chunk ^= (position & 0x3)
            elif i == 1:
                # Second chunk mixes with first chunk
                chunk ^= ((position & 0x7) | (prev_chunk & 0x3))
            else:
                # Later chunks get more mixing while preserving relationships
                mix_value = ((position + i) & 0x7) | ((prev_chunk << 1) & 0x6)
                chunk ^= mix_value & chunk_mask
            
            # Rotate based on position and chunk patterns
            if chunk_size > 1:
                rotate = ((position + i) % (chunk_size - 1)) + 1
                chunk = ((chunk << rotate) | (chunk >> (chunk_size - rotate))) & chunk_mask
            
            # Add to result and update previous chunk
            transformed |= (chunk << current_bit)
            prev_chunk = chunk
            current_bit += chunk_size
        
        # Final mixing based on position patterns
        if position % 5 == 0:
            transformed ^= (position & 0x3)
        elif position % 3 == 0:
            transformed ^= ((position >> 2) & 0x1)
