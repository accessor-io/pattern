"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 22:02:31
    ...
    def _apply_bit_permutation(self, value, position):
        ...
        if position < 67:
            ...
            if position < 71:
                ...
                chunk1 = (base >> (16 + shift)) & 0xffff
                chunk2 = base & ((1 << (16 + shift)) - 1)
                # Apply the transformation
                new_value = (chunk1 << (shift + 1)) | chunk2
                # Add position-specific bits
                new_value |= ((position - 66) << (8 + shift))
                # Preserve lower bits from base
                new_value |= (base & ((1 << shift) - 1))
            elif position < 80:
                ...
                # Basic pattern for positions around 90
                if position >= 88 and position <= 92:
                    shift = (position - 88) * 2
                    new_value = ((base << shift) | (base >> (32 - shift))) & ((1 << 48) - 1)
                    new_value |= (base & ((1 << (position % 8 + 8)) - 1))
            else:
