"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:58:12
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        def transform_base(v: int, p: int) -> int:
            # Initial seed values
            seeds = [0x8, 0x15, 0x31, 0x4c, 0xe0]
            
            # Evolve the function based on position
            if p < 5:
                return seeds[p % 5]
            
            # Each position evolves the function
            result = v
            for i in range(p % 5 + 1):
                # Mix with previous seeds
                result ^= seeds[i]
                # Rotate based on position
                rotate = (p + i) % 8
                result = ((result << rotate) | (result >> (32 - rotate))) & 0xffffffff
                # Apply position-specific transform
                result = (result * (0x1234567 + p)) & 0xffffffff
            
            return result
            
        # Apply the transformation
        x = transform_base(x, position)
        
        # Apply position-specific mixing
        if position >= 5:
            # Additional mixing for later positions
            x ^= (x >> (position % 7 + 1)) & ((1 << (32 - position % 8)) - 1)
            x = (x * (0x7b + position)) & 0xffffffff
            
            # Rotate bits based on position
            rotate = position % 11
            x = ((x << rotate) | (x >> (32 - rotate))) & 0xffffffff
