"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:23:39
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if (position - 4) % 5 == 0:  # positions 9, 14, 19, 24, 29, etc.
            ...
            # For positions beyond 85, use a pattern based on the last few special values
            if position > 85:
                last_special = max(k for k in special_values.keys())
                last_value = special_values[last_special]
                pattern_base = (position - last_special) * 0x1234567
                pattern_shift = ((position - last_special) * 5) % significant_bits
                result = ((last_value << pattern_shift) | (last_value >> (significant_bits - pattern_shift))) & ((1 << significant_bits) - 1)
                return (result + pattern_base) & ((1 << significant_bits) - 1)
            
        # For other positions, apply standard bit permutation with improved transitions
        significant_bits = self._get_significant_bits(position)
        ...
        # Get the previous and next special values
        special_values = {
            70: 0x349b84b6431a6c4ef1,
            75: 0x4c5ce114686a1336e07,
            80: 0xea1a5c66dcc11b5ad180,
        ...
        # Find the closest special values
        ...
        if next_special is not None:
            ...
            next_value = value << 5  # Grow by 5 bits
        
        # Calculate transition value based on position within block
        if block_pos == 1:
            # First position after special value
            transition = ((prev_value << 1) ^ next_value) & mask
        elif block_pos == 2:
            # Second position after special value
            transition = ((prev_value >> 2) + next_value) & mask
        elif block_pos == 3:
            # Third position after special value
            transition = ((prev_value + next_value) >> 1) & mask
        else:  # block_pos == 4
            # Fourth position before special value
            transition = (prev_value ^ (next_value << 2)) & mask
        
        # Apply additional transformations based on block number
        rotation = (block_num * 7 + block_pos * 3) % significant_bits
        result = ((transition << rotation) | (transition >> (significant_bits - rotation))) & mask
        
        # Apply non-linear transform
        transformed = self._apply_non_linear_transform(result, position)
