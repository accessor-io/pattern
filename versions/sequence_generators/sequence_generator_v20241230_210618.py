"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 21:06:18
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value, position):
        ...
        if position < 67:
            # Before position 67, use a more controlled growth
            shift = (position * 3) % 8
            chunk = (base >> shift) & 0xff
            new_value = ((new_value << 2) | (new_value >> 14)) & 0xffff
            new_value ^= (chunk << (position % 4))
            
            # Add position-dependent offset
            offset = (position * 17 + prev_bits * 3) % 16
            new_value = (new_value + offset) & 0xffff
        else:
        ...
        max_bits = min(prev_bits + (1 if position < 67 else 2), 16)
        mask = (1 << max_bits) - 1
