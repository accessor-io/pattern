"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 21:46:33
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value, position):
        ...
        if position < 67:
            if position < 20:
                # Early growth phase - roughly doubling with some bit preservation
                shift = (position % 4) + 1
                preserved = base & ((1 << shift) - 1)
                new_value = (base << 1) | preserved
                new_value += position * 0x31
            elif position < 40:
                # Middle phase - more complex growth with pattern preservation
                shift = (position % 8) + 4
                chunk1 = (base >> 16) & 0xffff
                chunk2 = base & 0xffff
                new_value = ((chunk1 << shift) | (chunk2 >> (16 - shift))) if shift < 16 else chunk1
                new_value = (new_value << 8) | (position * 0x51)
            elif position < 60:
                # Later phase - exponential growth with specific bit patterns
                preserved = base & 0xfffff
                multiplier = 1 + (position - 40) * 0.5
                new_value = int(base * multiplier)
                new_value |= preserved
            else:
                # Final approach to position 67
                preserved = base & 0xffffff
                new_value = (base << 2) | preserved
                new_value += position * 0x1234
        else:
            # After position 67, continue the pattern
            shift = position % 16
            preserved = base & 0xffffff
            new_value = ((base << shift) | (base >> (32 - shift))) if shift else base
            new_value = (new_value + position * 0x5678) & 0xffffffffffffffff
            
        # Ensure we don't exceed reasonable bounds while preserving patterns
        max_bits = min(prev_bits + (2 if position > 40 else 1), 64)
        mask = (1 << max_bits) - 1
