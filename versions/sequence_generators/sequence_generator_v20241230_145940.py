"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:59:40
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        def discover_pattern(v: int, p: int) -> int:
            ...
            # For later positions, discover pattern based on position
            block_num = (p - 2) // 5
            pos_in_block = (p - 2) % 5
            
            # Calculate base value using position-specific pattern
            if pos_in_block == 0:
                # First position in block uses previous block's last value
                base = seeds[len(seeds) - 1]
                # Apply block-specific transformation
                for i in range(block_num - 2):
                    base = (base * 0x7b + (i * 0x1234)) & 0xffffffff
                    base ^= (base >> 3) & 0xfffff
            else:
                # Other positions use values from same position in previous blocks
                base = seeds[10 + pos_in_block - 1]  # Start from third block's values
                # Apply position-specific transformation
                for i in range(block_num - 2):
                    base = (base * (0x1460 + i) + pos_in_block) & 0xffffffff
                    base ^= (base >> (pos_in_block + 2)) & 0x3ffff
            
            # Apply final mixing based on block number and position
            result = base
            result = (result * (0x2930 + block_num)) & 0xffffffff
            result ^= (result >> ((block_num % 5) + 3)) & 0xfffff
            
            # Rotate based on position in block
            rotate = pos_in_block * 3 + 1
            result = ((result << rotate) | (result >> (32 - rotate))) & 0xffffffff
            
            return result
            
        # Apply the pattern discovery
        x = discover_pattern(x, position)
