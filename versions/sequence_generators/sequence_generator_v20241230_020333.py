"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:03:33
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if position % 5 == 0:
            # For other multiples of 5, generate pattern based on position
            result = ['0'] * sig_bits
            
            # First bit is always 1
            result[0] = '1'
            
            # For positions divisible by 5, use a special pattern
            base = position // 5
            
            # Pattern 1: Every base bits, place a 1
            for i in range(0, sig_bits, base):
                if i < sig_bits:
                    result[i] = '1'
            
            # Pattern 2: XOR with previous values
            for i in range(1, sig_bits - 2, 3):
                if i + 2 < sig_bits:
                    # XOR with previous bits
                    prev = int(''.join(result[max(0, i-2):i]), 2) if i > 1 else 1
                    result[i] = '1' if bin(prev).count('1') % 2 == 0 else '0'
                    
                    # Position-based alternation
                    result[i+1] = '1' if ((i + position) % 3) == 0 else '0'
                    
                    # XOR of surrounding bits
                    surround = int(''.join(result[max(0, i-1):i+2]), 2)
                    result[i+2] = '1' if bin(surround).count('1') % 2 == 0 else '0'
            
            # Pattern 3: Position-based mixing
            for i in range(2, sig_bits - 1, base):
                if i < sig_bits:
                    # Mix with adjacent bits
                    window = result[max(0, i-2):min(i+3, sig_bits)]
                    ones = window.count('1')
                    result[i] = '1' if ones % 2 == 0 else '0'
