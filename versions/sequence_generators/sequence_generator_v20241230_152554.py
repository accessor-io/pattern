"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:25:54
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # For positions after initial values, use sophisticated pattern analysis
        ...
        # Get previous block's last value if available
        prev_block_last = None
        if len(self.sequence) >= block_size:
            prev_block_idx = ((position - 1) // block_size) * block_size + (block_size - 1)
            if prev_block_idx < len(self.sequence):
                prev_block_last = self.sequence[prev_block_idx]
        
        # Calculate base value using block-aware growth
        if block_position == 0:
            # Start of new block: use previous block's last value
            if prev_block_last is not None:
                base = prev_block_last * 2
            else:
                base = prev_value * 2
        else:
            # Within block: use relationship with previous value
            growth = 1.8 - (block_position * 0.1)  # Decreases within block
            base = int(prev_value * growth)
        
        # Apply sophisticated transformations based on block position
        if block_position == 0:
            # Start of block: establish new pattern
            shift = prev_high_bit // 3
            new_value = (base >> shift) | (base << 1)
        elif block_position == 1:
            # Second position: mix with block start
            block_start = self.sequence[position - 1]
            new_value = base ^ (block_start >> 2)
        elif block_position == 2:
            # Middle position: preserve structure
            mask = (1 << (prev_high_bit // 2)) - 1
            new_value = (base & ~mask) | (prev_value & mask)
        elif block_position == 3:
            # Fourth position: controlled variation
            variation = position * 0x111
            mask = (1 << prev_high_bit) - 1
            new_value = base ^ (variation & mask)
        else:
            # Last position: combine patterns
            if prev_block_last is not None:
                new_value = base | (prev_block_last & 0x7ff)
            else:
                new_value = base | (prev_value & 0x7ff)
        
        # Apply block number based transformations
        if block_number > 0 and prev_block_last is not None:
            # Mix with previous block's pattern
            shift = (block_number % 3) + 1
            new_value ^= prev_block_last >> shift
