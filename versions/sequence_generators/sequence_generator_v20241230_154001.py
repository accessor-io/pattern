"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:40:01
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value, position):
        ...
        # The pattern shows that each new value preserves some bits
        # from the previous value while growing gradually
        shift = (position * prev_bits) % 8  # Use small shifts for controlled growth
        new_value = value + (value << shift)
        
        # Ensure the value grows but doesn't exceed reasonable bounds
        max_bits = min(prev_bits + 1, 64)  # Allow very gradual growth
        mask = (1 << max_bits) - 1
