"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 03:18:48
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        # Dynamic shift based on position
        shift = (position * 7 + 3) % 64
        ...
        for i in range(64):
            ...
            src_pos = (i + shift) % 64
            if position % 2 == 0:
                # Even positions: rotate right
                result[i] = bits[src_pos]
            else:
                # Odd positions: rotate left and flip
                result[i] = '1' if bits[(63 - src_pos) % 64] == '0' else '0'
            
            # Apply additional transformations based on position
            if position % 5 == 0 and i % 8 == 0:
                # Every 5th position: special bit manipulation
                result[i] = '1' if bits[(i * 3 + position) % 64] == '1' else '0'
