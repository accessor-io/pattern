"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:46:55
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        block_num = (position - 8) // 5  # Which 5-string block we're in
        block_pos = (position - 8) % 5   # Position within the 5-string block (0-4)
        
        # Base value for transformations
        base = value
        
        # Apply block-specific transformations
        if block_pos == 0:  # First position in block - establish block pattern
            shift1 = (block_num * 3 + 7) % 32
            shift2 = (block_num * 5 + 11) % 32
            temp1 = (base << shift1) | (base >> (32 - shift1))
            temp2 = (base << shift2) | (base >> (32 - shift2))
            result = temp1 ^ temp2 ^ (block_num * 0x202)
            
        elif block_pos == 1:  # Second position - build on block pattern
            shift = (block_num * 7 + 13) % 32
            rotate = (block_num * 11 + 17) % 32
            temp1 = (base << shift) | (base >> (32 - shift))
            temp2 = (base << rotate) | (base >> (32 - rotate))
            result = (temp1 + temp2) ^ (block_num * 0x303)
            
        elif block_pos == 2:  # Middle position - peak complexity
            shift1 = (block_num * 13 + 19) % 32
            shift2 = (block_num * 17 + 23) % 32
            shift3 = (block_num * 19 + 29) % 32
            temp1 = (base << shift1) | (base >> (32 - shift1))
            temp2 = (base << shift2) | (base >> (32 - shift2))
            temp3 = (base << shift3) | (base >> (32 - shift3))
            result = temp1 ^ temp2 + temp3 + (block_num * 0x505)
            
        elif block_pos == 3:  # Fourth position - start transition
            rotate1 = (block_num * 23 + 31) % 32
            rotate2 = (block_num * 29 + 37) % 32
            temp1 = (base << rotate1) | (base >> (32 - rotate1))
            temp2 = (base << rotate2) | (base >> (32 - rotate2))
            result = (temp1 ^ temp2) + (block_num * 0x707)
            
        else:  # Last position - prepare for next block
            shift1 = (block_num * 31 + 41) % 32
            shift2 = (block_num * 37 + 43) % 32
            temp1 = (base << shift1) | (base >> (32 - shift1))
            temp2 = (base << shift2) | (base >> (32 - shift2))
            result = (temp1 + temp2) ^ (block_num * 0x909)
        
        # Apply position-specific adjustments
        result = result ^ (position * 0x101)
        result = (result << 3) | (result >> 29)  # Final rotation
