"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 01:36:29
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if position >= 8:
            # Preserve fewer bits with position-based variation
            preserve_bits = min(8, max(4, position % 7 + 3))  # Varies between 4-8 bits
            prev_bits = bits[:preserve_bits]
            for i in range(preserve_bits):
                result[i] = prev_bits[i]
        
        # Handle last byte with position-based variation
        last_byte = value & ((1 << (position % 8 + 4)) - 1)  # Variable mask
        last_byte_bits = format(last_byte, '08b')
        
        # Apply dynamic permutation
        for i in range(sig_bits - 8):
            src_pos = i % len(bits)
            # Use position-dependent factors
            p1 = (position % 13) + 7
            p2 = (position % 11) + 5
            new_pos = ((i * p1 + p2) * 3) % (sig_bits - 8)
            new_pos += 8
            result[new_pos] = bits[src_pos]
        
        # Ensure minimum hamming weight with variation
        result_str = ''.join(result)
        weight = result_str.count('1')
        target_weight = max(position % 6 + 2, sig_bits // 5)  # More variable target
        
        if weight < target_weight:
            # Add bits with position-based spacing
            spacing = (position % 3) + 2
            for i in range(sig_bits - 12, sig_bits - 4, spacing):
                if weight >= target_weight:
                    break
                if result[i] == '0':
                    result[i] = '1'
    ...
    def _apply_non_linear_transform(self, value: int, position: int) -> int:
        ...
        base_size = position % 5 + 4  # 4-8 bits
        chunk_sizes = [base_size]
        for i in range(3):
            chunk_sizes.append(base_size + (i % 3))  # Varying sizes
        
        chunks = []
        bit_pos = 0
        
        # Create variable-sized chunks with minimal overlap
        for size in chunk_sizes:
            chunk = (value >> bit_pos) & ((1 << size) - 1)
            chunks.append(chunk)
            bit_pos += size - 1  # Small overlap
        
        # Apply position-dependent transformations
        for i in range(len(chunks)):
            size = chunk_sizes[i]
            # Use position to vary operations
            shifts = [(position + i) % 5 + 2, (position + i + 1) % 3 + 2]
            for shift in shifts:
                chunks[i] = chunks[i] ^ (chunks[i] << shift)
                chunks[i] = chunks[i] ^ (chunks[i] >> (shift - 1))
                chunks[i] &= (1 << size) - 1
        
        # Recombine with position-based shifts
        result = 0
        bit_pos = 0
        for i, chunk in enumerate(chunks):
            shift = bit_pos
            result ^= (chunk << shift) & ((1 << sig_bits) - 1)
            bit_pos += chunk_sizes[i] - 1
        
        # Add final mixing based on position
        mix_shift = position % 7 + 1
