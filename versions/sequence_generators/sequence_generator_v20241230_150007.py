"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:00:07
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        def discover_pattern(v: int, p: int) -> int:
            ...
            # Get base value from the third block (positions 10-14)
            third_block = [0xa7b, 0x1460, 0x2930, 0x68f3, 0xc936]
            base = third_block[pos_in_block]
            
            # Apply transformations based on block number
            for i in range(block_num - 2):
                # Mix with position-specific values
                if pos_in_block == 0:
                ...
                # Apply block-specific mixing
                base = (base * (0x2930 + i)) & 0xffffffff
                base ^= (base >> ((i % 5) + 3)) & 0xfffff
