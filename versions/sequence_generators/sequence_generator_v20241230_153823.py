"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:38:23
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        if position <= 7:
            ...
            # After position 7, the growth is more gradual
            # We want to ensure the significant bits increase more slowly
            return min(48, 8 + int(position * 0.5))
    ...
    def _apply_bit_permutation(self, value, position):
        ...
        # Create a mask based on significant bits for this position
        significant_bits = self._get_significant_bits(position)
        mask = (1 << significant_bits) - 1
        
        # Apply the mask to keep only significant bits
