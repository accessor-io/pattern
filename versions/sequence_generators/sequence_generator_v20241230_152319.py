"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:23:19
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # For positions after initial values, use controlled growth pattern
        ...
        if position < 15:
            # Initial growth phase: roughly doubling
            growth = 2.0 - (position - 11) * 0.1  # Gradually decrease from 2.0
            base = int(prev_value * growth)
        else:
            # Later growth phase: more controlled
            growth = 1.5 - (position - 15) * 0.02  # Gradually decrease from 1.5
            growth = max(1.2, growth)  # Don't go below 1.2x
            base = int(prev_value * growth)
        
        # Apply position-dependent bit manipulations
        if position % 3 == 0:
            # Mix high and low bits
            new_value = base ^ (base >> (prev_high_bit // 2))
        elif position % 3 == 1:
        ...
        if new_value <= prev_value:
            new_value = prev_value + (prev_value >> 3)
