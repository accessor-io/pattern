"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:15:31
...
class SequenceGenerator:
    ...
    def generate_sequence(self, length: int) -> List[str]:
        ...
        print("=" * 120)
        print(f"{'Pos':>4} | {'Sig.Bits':>8} | {'Generated':>20} | {'Original':>20} | {'Diff':>20} | {'Match?'}")
        print("-" * 120)
        ...
        for i, (gen, orig) in enumerate(zip(hex_sequence, self.original_sequence[:length])):
            ...
            print(f"{i:4d} | {sig_bits:8d} | {gen_val:#x:>20} | {orig_val:#x:>20} | {diff:#x:>20} | {match} {' '.join(notes)}")
            ...
            if match == "✗":
                ...
                if i > 7:  # Only show binary for mismatches after initial values
                    print(f"{'':4} | {'':8} | Binary (gen):  {bin(gen_val)[2:]:>{sig_bits}} |")
                    print(f"{'':4} | {'':8} | Binary (orig): {bin(orig_val)[2:]:>{sig_bits}} |")
