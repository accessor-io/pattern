"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:13:51
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        ...
        # Base significant bits calculation
        if position < 15:
            return 8 + (position - 7)
        elif position < 30:
            return 16 + ((position - 14) // 2)
        elif position < 45:
            return 24 + ((position - 29) // 3)
        else:
            return 32 + ((position - 44) // 4)
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        # Split into variable-sized chunks based on position
        if position < 15:
            chunk_size = 4
        elif position < 30:
            chunk_size = 6
        ...
        num_chunks = (significant_bits + chunk_size - 1) // chunk_size
        ...
        for i in range(num_chunks):
            # Extract chunk
            shift = i * chunk_size
            chunk_mask = (1 << min(chunk_size, significant_bits - shift)) - 1
            chunk = (result >> shift) & chunk_mask
            ...
            if i == 0:
                ...
                chunk ^= ((position + i) & 0x7) | prev_chunk
            ...
            transformed |= (chunk << shift)
            prev_chunk = chunk
        
        # Final mixing based on position patterns
        if position % 5 == 0:
            transformed ^= (position & 0xf)
        elif position % 3 == 0:
            transformed ^= ((position >> 1) & 0x7)
