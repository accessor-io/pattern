"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:39:14
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value, position):
        ...
        # Calculate the new value based on position and previous value
        # The pattern shows that each new value builds on the previous one
        # by shifting and combining bits in a specific way
        shift = (position * prev_bits) % 16  # Use smaller shifts for more controlled growth
        new_value = value
        
        # Apply multiple transformations
        for i in range(4):  # Apply 4 different transformations
            shifted = (new_value << shift) & ((1 << (prev_bits + shift)) - 1)
            new_value ^= shifted
            new_value |= (value & ((1 << prev_bits) - 1))  # Preserve some bits from original
            
        # Ensure we don't grow too fast by masking
        max_bits = min(prev_bits + 2, 64)  # Allow gradual growth
        mask = (1 << max_bits) - 1
