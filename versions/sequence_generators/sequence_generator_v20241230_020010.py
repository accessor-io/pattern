"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 02:00:10
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if position % 5 == 0:
            ...
            # For other multiples of 5, use exponential growth pattern
            # Average growth rate is around 35x between consecutive 5th positions
            base = position // 5
            if base <= 6:
                # Use the known growth rates for first 30 positions
                growth_rates = [1, 23.6, 44.6, 34.9, 30.4, 38.6]
                result = 0x31  # Start with position 5's value
                for i in range(1, base):
                    result = int(result * growth_rates[i])
                return result & ((1 << sig_bits) - 1)
            else:
                # For positions beyond 30, use average growth rate
                result = 0x7d4fe747  # Start with position 30's value
                for i in range(6, base):
                    result = int(result * 35)
                return result & ((1 << sig_bits) - 1)
