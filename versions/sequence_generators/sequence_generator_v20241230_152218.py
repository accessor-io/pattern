"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:22:18
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # For positions after initial values, use bit pattern analysis
        ...
        if position % 5 == 0:
            # Shift pattern
            new_value = (prev_value << 1) | (prev_value >> prev_high_bit)
        elif position % 5 == 1:
            # Mix pattern
            new_value = prev_value ^ (prev_value >> 2)
        elif position % 5 == 2:
            # Growth pattern
            new_value = prev_value + (prev_value >> 3)
        elif position % 5 == 3:
            # Alternate pattern
            new_value = prev_value ^ ((position * 0x111) & 0xfff)
        else:
            # Combine pattern
            new_value = (prev_value << 2) | (prev_value >> (prev_high_bit - 1))
