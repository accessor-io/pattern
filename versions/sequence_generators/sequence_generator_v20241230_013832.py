"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 01:38:32
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        ...
        if position <= 7:
            ...
            growth = int(math.log2(position - 6) * 2.5)  # Slower growth
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if position >= 8:
            ...
            if position == 8:
                ...
                # Combine bits
                for i in range(9):
                    result[i] = prev_bits[i] if i < 4 else target_bits[i]
            else:
                # Normal case
        ...
        for i in range(mid_bits):
            src_pos = (i * 7 + position * 2) % len(bits)
        ...
        min_weight = max(sig_bits // 4, position // 3)
