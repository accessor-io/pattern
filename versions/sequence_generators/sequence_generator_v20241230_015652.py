"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 01:56:52
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        result = ['0'] * sig_bits
        bits = format(value, f'0{sig_bits}b')[-sig_bits:]
        
        # Preserve first few bits from previous value
        preserve_count = min(sig_bits - 2, 4)
        for i in range(preserve_count):
            result[i] = bits[i]
        
        # Fill remaining bits with position-based pattern
