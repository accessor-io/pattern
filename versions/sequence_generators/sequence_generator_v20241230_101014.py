"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:10:14
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        section_size = max(1, significant_bits // 4)
        num_sections = significant_bits // section_size
    ...
    def _apply_non_linear_transform(self, value: int, position: int) -> int:
        ...
        if sig_bits - 2 * chunk_size > 1:
            chunk3 = ((chunk3 << rotate) | (chunk3 >> ((sig_bits - 2 * chunk_size) - rotate))) & ((1 << (sig_bits - 2 * chunk_size)) - 1)
        
        # Mix chunks with position-based patterns
        result = chunk1
        result |= (chunk2 << chunk_size)
        result |= (chunk3 << (2 * chunk_size))
        
        # Apply final transformation based on position pattern
        rotate_final = (position % 3) + 1
        result = ((result << rotate_final) | (result >> (sig_bits - rotate_final))) & ((1 << sig_bits) - 1)
