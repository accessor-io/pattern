"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:27:51
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Dynamic growth rate based on position and previous value
        growth_factor = 1.0 + math.log2(position + 1) / prev_high_bit
        base = int(prev_value * growth_factor)
        
        # Dynamic bit manipulation based on position patterns
        pattern_shift = (position * prev_high_bit) % (prev_high_bit + 1)
        rotated = ((base << pattern_shift) | (base >> (prev_high_bit - pattern_shift))) & ((1 << (prev_high_bit + 4)) - 1)
        
        # Apply non-linear transformation
        salt = int(math.sqrt(position) * 0x1111)
        transformed = rotated ^ (salt & ((1 << (prev_high_bit + 2)) - 1))
        
        # Ensure growth while preserving interesting patterns
        if transformed <= prev_value:
            min_growth = math.log2(position + 2)
            transformed = prev_value + int(transformed % min_growth) + 1
