"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:14:27
...
class SequenceGenerator:
    ...
    def _get_significant_bits(self, position: int) -> int:
        ...
        if position < 12:
            ...
            return 12 + ((position - 11) * 3) // 4
        elif position < 36:
            return 20 + ((position - 23) * 4) // 5
        else:
            return 28 + ((position - 35) * 5) // 6
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        for i in range(num_chunks):
            if i == num_chunks - 1:
                ...
                size = min(base_chunk + (i % 2), remaining_bits)
        ...
        for i, chunk_size in enumerate(chunk_sizes):
            ...
            if i == 0:
                ...
                mix_value = ((position + i) & 0x7) | (prev_chunk & 0x3)
                chunk ^= mix_value & chunk_mask
            
            # Rotate based on position and chunk patterns
            rotate = ((position + i) % (chunk_size - 1)) + 1 if chunk_size > 1 else 0
            if rotate:
        ...
        if position % 5 == 0:
            transformed ^= (position & 0x7)
        elif position % 3 == 0:
            transformed ^= ((position >> 2) & 0x3)
