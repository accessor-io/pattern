"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:50:08
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Position-specific bit patterns
        if pos_in_block == 0:
            # First position: Initialize pattern
            x = (x << 3) | (x >> 29)
            x = x + ((block * 8) & 0xF)
            
        elif pos_in_block == 1:
            # Second position: Build pattern
            x = (x << 2) | (x >> 30)
            x = x + ((block * 16) & 0x1F)
            
        elif pos_in_block == 2:
            # Middle position: Peak pattern
            x = (x << 4) | (x >> 28)
            x = x + ((block * 32) & 0x3F)
            
        elif pos_in_block == 3:
            # Fourth position: Descend pattern
            x = (x << 1) | (x >> 31)
            x = x + ((block * 16) & 0x1F)
            
        else:
            # Last position: Reset pattern
            x = (x << 3) | (x >> 29)
            x = x + ((block * 8) & 0xF)
