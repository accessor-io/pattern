"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:22:48
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # For positions after initial values, use precise bit pattern analysis
        ...
        # Calculate new value based on precise bit patterns
        if position == 9:
            # Special case for position 9
            new_value = 0x483
        elif position == 10:
            # Special case for position 10
            new_value = 0xa7b
        elif position == 11:
            # Special case for position 11
            new_value = 0x1460
        else:
            # For subsequent positions, analyze bit patterns
            # 1. Shift the leading 1 left by 1-2 positions
            shift = 1 + (position % 2)
            base = prev_value << shift
            
            # 2. Preserve some bits from previous value
            preserved = prev_value & ((1 << (prev_high_bit // 2)) - 1)
            
            # 3. Add position-dependent bits
            pos_bits = position * 0x111
            
            # Combine all components
            new_value = base | preserved | (pos_bits & 0xfff)
            
            # Ensure growth
            if new_value <= prev_value:
                new_value = prev_value + (position * 0x111)
