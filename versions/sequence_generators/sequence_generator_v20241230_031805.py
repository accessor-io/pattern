"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 03:18:05
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if prev_special and next_special:
            ...
            weighted_ratio = 1 / (1 + math.exp(-6 * (ratio - 0.5)))
            
            # Extract significant bits patterns with better preservation
            prev_pattern = prev_value & ((1 << significant_bits) - 1)
            next_pattern = next_value & ((1 << significant_bits) - 1)
            
            # Calculate base pattern using weighted interpolation
            base_pattern = int(prev_pattern * (1 - weighted_ratio) + next_pattern * weighted_ratio) & mask
            ...
            if block_pos == 0:
                ...
                result = base_pattern ^ (base_pattern >> 2) ^ (position * 0x1234567) & mask
            elif block_pos == 3:
                # Progressive transition
                prog_ratio = (position - prev_special) / (next_special - prev_special)
                result = int(base_pattern * (1 - prog_ratio) + (base_pattern ^ 0x1234567) * prog_ratio) & mask
            else:  # block_pos == 4
                # Special handling for pre-transition
                next_shift = (next_special - position) % 4 + 1
                result = ((base_pattern << next_shift) ^ (base_pattern >> (significant_bits - next_shift))) & mask
