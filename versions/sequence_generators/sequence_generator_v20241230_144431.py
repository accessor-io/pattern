"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 14:44:31
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        # Use full 32-bit width
        mask = (1 << 32) - 1
        seed = value & mask
        
        # Different algorithms for each position within the 5-string block
        if block_pos == 0:  # First position in block
            shift = (block_num * 3 + 7) % 32
            temp = ((seed << shift) | (seed >> (32 - shift))) & mask
            result = (temp + (block_num * 0x202)) & mask
            
        elif block_pos == 1:  # Second position
            shift1 = (block_num * 2 + 5) % 32
            shift2 = (block_num * 3 + 11) % 32
            temp1 = ((seed << shift1) | (seed >> (32 - shift1))) & mask
            temp2 = ((seed << shift2) | (seed >> (32 - shift2))) & mask
            result = (temp1 ^ temp2) & mask
            
        elif block_pos == 2:  # Middle position
            rotate = (block_num * 7 + 13) % 32
            temp = ((seed << rotate) | (seed >> (32 - rotate))) & mask
            result = (temp + (block_num * 0x483)) & mask
            
        elif block_pos == 3:  # Fourth position
            shift = (block_num * 5 + 17) % 32
            temp = ((seed << shift) | (seed >> (32 - shift))) & mask
            result = (temp ^ (block_num * 0x405)) & mask
            
        else:  # Last position in block
            shift1 = (block_num * 11 + 19) % 32
            shift2 = (block_num * 13 + 23) % 32
            temp1 = ((seed << shift1) | (seed >> (32 - shift1))) & mask
            temp2 = ((seed << shift2) | (seed >> (32 - shift2))) & mask
            result = (temp1 + temp2) & mask
        
        # Final position-based adjustment
        result = (result + (position * 7)) & mask
        return result
