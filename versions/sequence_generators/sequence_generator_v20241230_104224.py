"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 10:42:24
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        if block_pos == 0:
            ...
            shift1 = (block_num * 3 + 2) % significant_bits
            shift2 = (block_num * 2 + 3) % significant_bits
            result = ((seed << shift1) ^ (seed >> shift2)) & mask
            result = (result + (block_num * 4)) & mask
        else:
            # For other positions, use a mix of shifts and XORs
            shift1 = (block_pos * 3 + 3) % significant_bits
            shift2 = (block_pos * 2 + 2) % significant_bits
            result = ((seed << shift1) ^ (seed >> shift2)) & mask
