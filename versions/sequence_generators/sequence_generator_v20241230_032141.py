"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 03:21:41
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, position, value):
        ...
        # Enhanced bit permutation logic
        result = value & mask
        ...
        if block_pos == 0:
            shift = (block_num * 3) % significant_bits
            result = ((result << shift) | (result >> (significant_bits - shift))) & mask
        elif block_pos == 1:
            result = (result ^ (block_num * 0x11)) & mask
        elif block_pos == 2:
            result = (result + (block_num * 0x22)) & mask
        elif block_pos == 3:
            result = (result ^ (block_num * 0x33)) & mask
        else:  # block_pos == 4
            result = (result + (block_num * 0x44)) & mask
        
        # Apply final transformation
        if position > 8:
            rotation = position % 7
            result = ((result << rotation) | (result >> (significant_bits - rotation))) & mask
            
            # Additional mixing for higher positions
            if position > 50:
                result ^= (position * 0x55)
            elif position > 30:
                result ^= (position * 0x33)
            elif position > 15:
                result ^= (position * 0x22)
