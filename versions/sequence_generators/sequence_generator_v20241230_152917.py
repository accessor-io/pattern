"""
SEQUENCE GENERATOR - Version saved at 2024-12-30 15:29:17
...
class SequenceGenerator:
    ...
    def _apply_bit_permutation(self, value: int, position: int) -> int:
        ...
        def highest_bit_pos(n):
            return max(1, len(bin(n)) - 3)
        
        prev_high_bit = highest_bit_pos(prev_value)
        
        # Calculate base value using bit patterns
        base = prev_value << 1
        if position % 3 == 0:
            base |= (prev_value >> (prev_high_bit // 2))
        elif position % 3 == 1:
            base ^= (prev_value >> (prev_high_bit // 3))
        else:
            base ^= (position * 0x1111)
        
        # Apply position-based transformations
        transformed = base
        if position > 8:
            # Use the pattern of differences to guide the transformation
            pattern_shift = (position * prev_high_bit) % 64
            transformed ^= (transformed >> pattern_shift) | (transformed << (64 - pattern_shift))
        
        # Ensure the value grows
        if transformed <= prev_value:
            transformed = prev_value + (position * 0x111)
